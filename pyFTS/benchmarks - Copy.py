...
def mape(targets, forecasts):
    return np.mean(abs(targets-forecasts)/targets) * 100
...
def compareModelsTable(original, models, forecasts):
    ...
    columns = ['Modelo', 'Partições-FS', 'RMSE', 'MAPE (%)']
    rows = []
    count = 0
    for model in models:
        nome = model.name
        error_r = rmse(original, np.array(forecasts[count]))
        error_m = round(mape(original, np.array(forecasts[count]))*100, 2)
        rows.append([nome, len(model.sets), error_r, error_m])
        count += 1
